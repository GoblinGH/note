...
#### 2.2 class和style属性
    ...
    <!-- 方法1：变量形式 -->
      ...
      <!-- 方法2：数组形式，同时引用多个 -->
      <p :class="[bb,dd]">北京欢迎你！</p>

      <!-- 方式3：json形式,最常用-->
      <p :class="{aa:true,cc:false}">北京欢迎你！</p>
      <p :class="{aa:true,cc:flag}">北京欢迎你！</p>
      <p :class="{aa:true,cc:num > 5 ? true : false }">北京欢迎你！</p>
      <p :class="num > 5 ? ee : dd">Vue欢迎你！</p>
      <p :class="{ff:num > 0 }">Vue！num测试</p>

      <!-- 方式4：变量引用json形式 -->
      <p :class="hello">Vue！num测试2</p>
...
### 2. 数据绑定的方式
    ...
    b.单向绑定    
        ...
        (v-clok必须配置css样式，否则不生效) 例如：[v-cloak]{display:none;}
        方式2：使用v-text、v-html
